...
- **Photo stamp is opt-in per field.** The door-photo field sets `"stamp_photos": true` so the server burns readable date, time, and GPS onto exported JPEGs from capture metadata. **California servers:** from January 1, 2027, CCP 417.10 (AB 747) requires a readable stamp on the door photo — keep `"stamp_photos": true` on that field. Gallery picks without EXIF may show date/time only. The punch is corroboration; the affidavit is still yours to swear.
