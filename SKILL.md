...
Store the raw key in `attempts.outcome`. `show_if` is documented as web-only, so the phone may show "Physical description" unconditionally; harmless. A submission with the door photo bills $0.15 instead of $0.05 (the photo is required, so plan on $0.15).
